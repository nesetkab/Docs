...
We need to adjust multipliers that convert encoder ticks into real-world measurements: inches. This ensures your localizer's readings are accurate.
    ...
    7. Input this value in `Constants` replacing the `.strafeTicksToInches(.001989436789)` with` .strafeTicksToInches([multiplier])`, where `[multiplier]` is the value you obtained from the tuner.
