...
<Callout title="Prerequisites" type="info">
  Make sure you have three odometry pods, two parallel pods and one strafe pods 
  plugged into the encoder ports for your motors on the Control Hub. Due to technical
  limitations of the Control Hub, encoder ports number 0 and 3 are the fastest and should
  be used for your two parallel pods.
</Callout>

## Default Values
In `Constants.java`, add an instance of `ThreeWheelConstants`. Make sure to replace the hardware map names 
with the actual names. You must also set the IMU orientation to match the orientation of your Control Hub.
