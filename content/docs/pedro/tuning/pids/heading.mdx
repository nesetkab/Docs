...
3. Select the `Tuning` Opmode. Use your gamepad to select the `Manual` folder. Then, select `Heading Tuner`.
...
<Note>
<strong>Warning:</strong> After adjusting a value in Panels, <strong>hit "enter"</strong> in order to save it and cause the robot to correct differently. However, any values you modify through Panels <strong>are not saved into your code!</strong> In order to transfer the values you just tuned on Panels into your code, go to the <strong>Update Tuned Values</strong> section to learn more.
</Note>
