...
1. Open Panels. If you haven't used Panels before, you can read their documentation on the "Panels" tab of this website.
2. On your Driver Hub or Driver Station, and connect a gamepad to it. Make sure to press "start" + "a" on the gamepad. 
3. Select the `Tuning` Opmode. Use your gamepad to select `Manual`. Then, select `Translational`. 
4. Ensure that the timer for autonomous OpModes is **disabled.** Otherwise, the OpMode will automatically stop after 30 seconds.
5. Run the run the `Translational` tuner autonomous OpMode.
