...
2. Adjust the PIDF constants (`drivePIDFCoefficients`) in the FollowerConstants tab of Panels to ensure that the robot smoothly and accurately drives straight back and forth. 
...
**If you have a dual PIDF system enabled,** it is recommended to first tune the main PIDF, `drivePIDFCoefficients` before tuning the secondary PIDF, `secondaryDrivePIDFCoefficients`. 

<Callout title="Tuning Tips" type="info">
This is also a great time to test how well your previously tuned Translational and Heading PIDFs work together. If the robot's movements are too jittery while driving along its path, you may want to consider re-tuning the Translational and/or Heading PIDFs.
</Callout>
