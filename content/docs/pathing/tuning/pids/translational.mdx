...
**If you have a dual PIDF system enabled,** it is recommended to first tune the main PIDF, `translationalPIDFCoefficients`, to ensure that the robot can correct from large errors and bring it within the secondary PID's range. Then, tune the secondary PIDF, `secondaryTranslationalPIDFCoefficients`, so the robot can smoothly correct from smaller errors and minimize oscillations. 
...
If additional feedforward is needed, use the feedforward term directly in the `translationalPIDFCoefficients` and/or `secondaryTranslationalPIDFCoefficients` if 
